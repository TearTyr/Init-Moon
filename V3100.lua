...
do
	_g.ghost_part = function(part, parent)
		...
		newPart.Size = newPart.Size * 1.01
	...
	_g.RobloxToSimple = function(roblox, rig, fps)
		...
		if type(roblox) == "number" then
			...
			if not succ then warn(err) return nil, "failed to insert animation from id" end
		...
		local function add_buffers(target, data, tween, default)
			...
			for _, kfPos in pairs(all_pos) do
				...
				if kfPos == 0 then
					...
					local ease = data.Easing[kfPos] and Ease.Detableize(data.Easing[kfPos]) or Ease.LINEAR()
					for bufferPos = ini_pos, kfPos, 1 do
						buffer_tbl.Buffer[bufferPos] = tween(ini_value, final_value, ease._func((bufferPos - ini_pos) / dist))
					end
					ease:Destroy()
				end
	...
	_g.ExportItemObject = function(ItemObject)
		...
		if ItemObject.RigContainer then
			...
			if #roblox_kfSeq:GetChildren() > 0 then
...
do
	...
	_g.Themer:SetTheme(_g.Themer._BuiltInThemes.Default)
end
------------------------------------------------------------
do
	...
	function activate()
		...
		local SavedTheme = _g.plugin:GetSetting("MoonAnimator2_SavedTheme")
